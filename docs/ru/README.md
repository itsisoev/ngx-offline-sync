...
Добавлена поддержка приоритетов для управления порядком синхронизации offline-запросов.

* `HIGH`, `NORMAL` и `LOW` приоритеты.
* `NORMAL` используется по умолчанию.
* Приоритет можно задавать для отдельного HTTP-запроса через `HttpContext`.
* Запросы с более высоким приоритетом синхронизируются раньше запросов с более низким.
* Запросы с одинаковым приоритетом сохраняют исходный порядок в очереди.
...
Добавлена автоматическая обработка временных ошибок.

* Настраиваемые `maxAttempts` и `delay`.
* Linear Backoff между повторными попытками.
* Автоматическое планирование повторных попыток.
* Retry для сетевых ошибок и ошибок сервера `5xx`.
...
Расширена система логирования и мониторинга синхронизации.

* Настраиваемые уровни логирования через `LogLevel`.
* Поддержка английского и русского языков через `LogLanguage`.
* Типизированные `LogEvent`.
